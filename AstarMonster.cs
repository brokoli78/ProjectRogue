...
{
    public static class AStarMonster
    {
        static Creature me;
        ...
        {
            static Exception InavlidNodeCostExeption = new Exception("Duplicate Node");
            ...
            internal MapNode(TileMap map, int x, int y, double cost)
            {
                ...
                this.x = x;
                this.y = y;
                if (map[x, y].creature == null || map[x,y].creature == me || map[x,y].creature == GameController.player)
                {
                    this.cost = cost;
                }
                else
